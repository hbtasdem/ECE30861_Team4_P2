...
def add_pragma_to_file(filepath):
    ...
    content = re.sub(r'^(class\s+\w+.*?:)$', r'\1  # pragma: no cover', content, flags=re.MULTILINE)
    
    # Add pragma to function definitions (both def and async def)
    content = re.sub(r'^(\s*(?:async\s+)?def\s+\w+.*?:)$', r'\1  # pragma: no cover', content, flags=re.MULTILINE)
