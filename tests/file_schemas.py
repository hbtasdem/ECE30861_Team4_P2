...
class FileMetadata(BaseModel):
    ...
    mime_type: str = Field(..., description="MIME type (e.g., 'application/octet-stream')")
...
class FileUploadRequest(BaseModel):
    ...
    tags: Optional[list[str]] = Field(None, description="Optional file tags for categorization")
...
class BatchFileUploadRequest(BaseModel):
    ...
    artifact_id: str = Field(..., description="ULID of the artifact")
    files: list[FileUploadRequest] = Field(..., min_length=1, description="List of files to upload")
...
class BatchFileUploadResponse(BaseModel):
    ...
    files: list[FileUploadResponse] = Field(..., description="Successfully uploaded files")
    errors: Optional[list[dict[str, str]]] = Field(None, description="Errors for failed uploads")
...
class DuplicateCheckRequest(BaseModel):
    ...
    artifact_id: Optional[str] = Field(None, description="Exclude this artifact from results")
...
class DuplicateCheckResponse(BaseModel):
    ...
    duplicate_file_id: Optional[str] = Field(None, description="ULID of duplicate file if found")
    duplicate_artifact_id: Optional[str] = Field(None, description="Artifact ID of duplicate if found")
    original_filename: Optional[str] = Field(None, description="Original filename of duplicate")
    uploaded_at: Optional[datetime] = Field(None, description="Upload date of duplicate")
...
class UploadSessionResponse(BaseModel):
    ...
    progress_percent: int = Field(..., description="Upload progress percentage", ge=0, le=100)
    status: str = Field(..., description="Upload status (pending, in_progress, completed, failed)")
...
class BatchUploadItem(BaseModel):
    ...
    metadata: Optional[dict[str, str]] = Field(None, description="Optional custom metadata")
...
class BatchUploadRequest(BaseModel):
    ...
    artifact_id: str = Field(..., description="ULID of the artifact")
    files: list[BatchUploadItem] = Field(..., min_length=1, max_length=50, description="List of files to upload (1-50)")
    skip_duplicates: bool = Field(default=False, description="Skip files with duplicate checksums")
    stop_on_error: bool = Field(default=False, description="Stop batch if any file fails")
...
class ChunkedUploadInitRequest(BaseModel):
    ...
    chunk_size_bytes: int = Field(default=10_485_760, description="Chunk size in bytes (default 10MB)", ge=262_144)
    total_chunks: int = Field(..., description="Total number of chunks")
    sha256_checksum: Optional[str] = Field(None, description="Optional pre-calculated SHA256")
    description: Optional[str] = Field(None, description="Optional file description")
...
class ChunkUploadResponse(BaseModel):
    ...
    progress_percent: int = Field(..., description="Overall progress percentage", ge=0, le=100)
...
class ChunkedUploadFinalizeResponse(BaseModel):
    ...
    verification_status: str = Field(..., description="Verification status (passed, failed, skipped)")
...
class FileValidationRequest(BaseModel):
    ...
    validation_type: str = Field(..., description="Type: mime_type, size, malware, metadata")
    options: Optional[dict[str, object]] = Field(None, description="Validation-specific options")
...
class FileValidationResult(BaseModel):
    ...
    details: Optional[dict[str, object]] = Field(None, description="Additional validation details")
...
class FileValidationResponse(BaseModel):
    ...
    validations: list[FileValidationResult] = Field(..., description="Results for each validation")
...
class UploadProgressResponse(BaseModel):
    ...
    status: str = Field(..., description="Status: pending, in_progress, completed, failed, paused")
