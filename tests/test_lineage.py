...
class TestGetModelConfig:
    ...
    def test_fetch_with_full_url(self):
        ...
        config = get_model_config("https://huggingface.co/google-bert/bert-base-uncased")
        assert config is not None
        assert "id" in config or "modelId" in config
    ...
    def test_handles_url_with_tree_path(self):
        ...
        config = get_model_config("https://huggingface.co/openai/whisper-tiny/tree/main")
        assert config is not None
...
class TestCheckLineage:
    ...
    def test_lineage_info_structure(self):
        ...
        assert lineage_info is not None
        required_fields = ["model", "base_model", "has_lineage", "lineage_depth", "all_parents"]
...
class TestExtractLineageGraph:
    ...
    def test_dataset_edges_created(self):
        ...
        dataset_edges = [e for e in graph["edges"] if "dataset" in e.get("relationship", "").lower() or "trained_on" in e.get("relationship", "").lower()]
